...
// Replace rsr-template-repo, jaffascript, {{DEPS}} with actuals
...
| `$PREFIX/share/jaffascript/`
| Data files, assets

| `$PREFIX/share/doc/jaffascript/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/jaffascript/config.toml`

Fallback: `$HOME/.config/jaffascript/config.toml`
...
Or via OPSM: `opsm update jaffascript`
...
[source,bash]
----
just install --prefix=/opt/jaffascript-instance1 --config=/etc/jaffascript/instance1.toml
just install --prefix=/opt/jaffascript-instance2 --config=/etc/jaffascript/instance2.toml
----
